...
- **Superseded by:** Checkpoint 4 below.
...
- **What it does show, carefully stated:** the pipeline can detect the exact dissociation the research question rests on — a risk signal that is informative about correctness (AUC 0.70, CI excluding 0.5) sitting alongside repair values that are uniformly zero. That is the same shape as the project's published GSM8K failure, where the heuristic escalated 57.8% of queries and lost accuracy. The mock reproduces the shape without licensing any claim about its cause.
- **GSM8K failure-mode analysis (required by the gate):** *Does high Triage risk predict wrong answers?* Yes, AUC 0.697 [0.628, 0.776]. *Does it predict that escalation improves the answer?* Unanswerable here — escalation improved nothing, so there is no target. *Do retrieval, resampling, verification or abstention help instead?* No: all exactly 0.000, abstention −0.050. *How often is each action's marginal value negative?* 0% for every action except abstain at 5%.
- **Defect found and fixed:** the first version of the probe reported **AUC 1.000 on three test rows** and a verdict of EVALUABLE. `predict_repair` now refuses below 20 test rows or 5 per class, and the verdict rule now requires response-only features to actually **beat** prompt-only rather than merely to have run.
